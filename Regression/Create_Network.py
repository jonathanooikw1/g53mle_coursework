...
def calc_metrics_2d_array(predictions, actual):
    ...
    print("Class two precision:", format(class_2_precision, '.2f') + "%", "Class two recall",
          format(class_2_recall, '.2f') + "%")
    print("Class three precision:", format(class_3_precision, '.2f') + "%", "Class three recall",
          format(class_3_recall, '.2f') + "%")
    print("Class four precision:", format(class_4_precision, '.2f') + "%", "Class four recall",
          format(class_4_recall, '.2f') + "%")
    print("Class five precision:", format(class_5_precision, '.2f') + "%", "Class five recall",
          format(class_5_recall, '.2f') + "%")
...
def generate_batch(batch_size):
    ...
    batch_labels = np.ndarray(shape=(batch_size, 3), dtype=np.intc)
...
loss_op = tf.reduce_mean(tf.keras.losses.mean_squared_error(neural_network, Y))
...
with tf.Session() as sess:
    ...
    for epoch in range(int(checkpoint), number_epochs):
        ...
        if epoch % 10 == 0:
            c = sess.run(loss_op, feed_dict={X: X_test, Y: y_test})
            ...
            print("Test accuracy: ", format(100 - np.sum(difference) / (len(y_test) * 5) * 100, '.2f') + "%")
